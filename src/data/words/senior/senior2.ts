import { Word } from "../types";

// 高校2年レベル（中上級語彙）- 35語
export const senior2Words: Word[] = [
  {
    id: 2101,
    word: "acknowledge",
    meaning: "認める、承認する",
    example: "I acknowledge my mistake.",
    exampleJa: "私は自分の間違いを認めます。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["communication", "business"],
    frequencyRank: "A",
    partOfSpeech: "verb",
    pronunciation: "/əkˈnɑːlɪdʒ/",
  },
  {
    id: 2102,
    word: "acquire",
    meaning: "獲得する、習得する",
    example: "She acquired a new skill.",
    exampleJa: "彼女は新しいスキルを習得しました。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "school"],
    frequencyRank: "A",
    partOfSpeech: "verb",
    pronunciation: "/əˈkwaɪər/",
  },
  {
    id: 2103,
    word: "adequate",
    meaning: "適切な、十分な",
    example: "Is this room adequate for the meeting?",
    exampleJa: "この部屋は会議に適切ですか？",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "daily"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/ˈædɪkwət/",
  },
  {
    id: 2104,
    word: "alternative",
    meaning: "代替の、選択肢",
    example: "We need to find an alternative solution.",
    exampleJa: "代替の解決策を見つける必要があります。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "communication"],
    frequencyRank: "A",
    partOfSpeech: "noun",
    pronunciation: "/ɔːlˈtɜːrnətɪv/",
  },
  {
    id: 2105,
    word: "anticipate",
    meaning: "予期する、期待する",
    example: "We anticipate strong sales this year.",
    exampleJa: "今年は売上が好調になると予期しています。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "communication"],
    frequencyRank: "A",
    partOfSpeech: "verb",
    pronunciation: "/ænˈtɪsɪpeɪt/",
  },
  {
    id: 2106,
    word: "approximate",
    meaning: "おおよその",
    example: "What is the approximate cost?",
    exampleJa: "おおよその費用はいくらですか？",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "finance"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/əˈprɑːksɪmət/",
  },
  {
    id: 2107,
    word: "authority",
    meaning: "権威、当局",
    example: "The authorities are investigating.",
    exampleJa: "当局が調査しています。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["communication", "business"],
    frequencyRank: "A",
    partOfSpeech: "noun",
    pronunciation: "/əˈθɔːrəti/",
  },
  {
    id: 2108,
    word: "capacity",
    meaning: "容量、能力",
    example: "The stadium has a capacity of 50,000.",
    exampleJa: "そのスタジアムの収容人数は5万人です。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "technology"],
    frequencyRank: "A",
    partOfSpeech: "noun",
    pronunciation: "/kəˈpæsəti/",
  },
  {
    id: 2109,
    word: "comprehensive",
    meaning: "包括的な",
    example: "We need a comprehensive plan.",
    exampleJa: "包括的な計画が必要です。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic800", displayName: "TOEIC800" },
    ],
    difficulty: 5,
    categories: ["business", "communication"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/ˌkɑːmprɪˈhensɪv/",
  },
  {
    id: 2110,
    word: "concentrate",
    meaning: "集中する",
    example: "I need to concentrate on my work.",
    exampleJa: "仕事に集中する必要があります。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
    ],
    difficulty: 4,
    categories: ["daily", "school"],
    frequencyRank: "A",
    partOfSpeech: "verb",
    pronunciation: "/ˈkɑːnsəntreɪt/",
  },
  {
    id: 2111,
    word: "confident",
    meaning: "自信がある",
    example: "I am confident we will succeed.",
    exampleJa: "私たちは成功すると確信しています。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic600", displayName: "TOEIC600" },
    ],
    difficulty: 4,
    categories: ["emotion", "communication"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/ˈkɑːnfɪdənt/",
  },
  {
    id: 2112,
    word: "conscious",
    meaning: "意識している",
    example: "I am conscious of the risks.",
    exampleJa: "リスクを意識しています。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
    ],
    difficulty: 4,
    categories: ["daily", "health"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/ˈkɑːnʃəs/",
  },
  {
    id: 2113,
    word: "considerable",
    meaning: "かなりの",
    example: "There has been considerable progress.",
    exampleJa: "かなりの進歩がありました。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "communication"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/kənˈsɪdərəbəl/",
  },
  {
    id: 2114,
    word: "consistent",
    meaning: "一貫した",
    example: "Be consistent in your approach.",
    exampleJa: "アプローチに一貫性を持ってください。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "communication"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/kənˈsɪstənt/",
  },
  {
    id: 2115,
    word: "controversial",
    meaning: "物議を醸す、議論の余地がある",
    example: "This is a controversial topic.",
    exampleJa: "これは物議を醸すトピックです。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
    ],
    difficulty: 5,
    categories: ["communication", "culture"],
    frequencyRank: "B",
    partOfSpeech: "adjective",
    pronunciation: "/ˌkɑːntrəˈvɜːrʃəl/",
  },
  {
    id: 2116,
    word: "conventional",
    meaning: "従来の、伝統的な",
    example: "We used conventional methods.",
    exampleJa: "従来の方法を使いました。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "culture"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/kənˈvenʃənəl/",
  },
  {
    id: 2117,
    word: "crucial",
    meaning: "極めて重要な",
    example: "This is a crucial decision.",
    exampleJa: "これは極めて重要な決断です。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "communication"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/ˈkruːʃəl/",
  },
  {
    id: 2118,
    word: "decline",
    meaning: "減少する、断る",
    example: "Sales declined last quarter.",
    exampleJa: "前四半期に売上が減少しました。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic600", displayName: "TOEIC600" },
    ],
    difficulty: 4,
    categories: ["business", "finance"],
    frequencyRank: "A",
    partOfSpeech: "verb",
    pronunciation: "/dɪˈklaɪn/",
  },
  {
    id: 2119,
    word: "derive",
    meaning: "由来する、得る",
    example: "The word derives from Latin.",
    exampleJa: "その単語はラテン語に由来します。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
    ],
    difficulty: 5,
    categories: ["school", "culture"],
    frequencyRank: "B",
    partOfSpeech: "verb",
    pronunciation: "/dɪˈraɪv/",
  },
  {
    id: 2120,
    word: "distinguish",
    meaning: "区別する",
    example: "Can you distinguish the difference?",
    exampleJa: "違いを区別できますか？",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
    ],
    difficulty: 5,
    categories: ["school", "daily"],
    frequencyRank: "A",
    partOfSpeech: "verb",
    pronunciation: "/dɪˈstɪŋɡwɪʃ/",
  },
  {
    id: 2121,
    word: "domestic",
    meaning: "国内の、家庭の",
    example: "Domestic flights are cheaper.",
    exampleJa: "国内線はより安いです。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic600", displayName: "TOEIC600" },
    ],
    difficulty: 4,
    categories: ["travel", "business"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/dəˈmestɪk/",
  },
  {
    id: 2122,
    word: "eliminate",
    meaning: "排除する、取り除く",
    example: "We need to eliminate waste.",
    exampleJa: "無駄を排除する必要があります。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "technology"],
    frequencyRank: "A",
    partOfSpeech: "verb",
    pronunciation: "/ɪˈlɪmɪneɪt/",
  },
  {
    id: 2123,
    word: "emphasis",
    meaning: "強調",
    example: "The emphasis is on quality.",
    exampleJa: "品質に重点が置かれています。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "communication"],
    frequencyRank: "A",
    partOfSpeech: "noun",
    pronunciation: "/ˈemfəsɪs/",
  },
  {
    id: 2124,
    word: "enormous",
    meaning: "巨大な",
    example: "This is an enormous opportunity.",
    exampleJa: "これは巨大なチャンスです。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
    ],
    difficulty: 4,
    categories: ["daily", "communication"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/ɪˈnɔːrməs/",
  },
  {
    id: 2125,
    word: "entirely",
    meaning: "完全に",
    example: "I entirely agree with you.",
    exampleJa: "あなたに完全に同意します。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
    ],
    difficulty: 4,
    categories: ["communication", "opinion"],
    frequencyRank: "A",
    partOfSpeech: "adverb",
    pronunciation: "/ɪnˈtaɪərli/",
  },
  {
    id: 2126,
    word: "essential",
    meaning: "不可欠な",
    example: "Water is essential for life.",
    exampleJa: "水は生命に不可欠です。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic600", displayName: "TOEIC600" },
    ],
    difficulty: 4,
    categories: ["daily", "health"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/ɪˈsenʃəl/",
  },
  {
    id: 2127,
    word: "eventually",
    meaning: "最終的に",
    example: "He eventually became a doctor.",
    exampleJa: "彼は最終的に医者になりました。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
    ],
    difficulty: 4,
    categories: ["daily", "communication"],
    frequencyRank: "A",
    partOfSpeech: "adverb",
    pronunciation: "/ɪˈventʃuəli/",
  },
  {
    id: 2128,
    word: "experiment",
    meaning: "実験",
    example: "We conducted an experiment.",
    exampleJa: "私たちは実験を行いました。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
    ],
    difficulty: 4,
    categories: ["school", "technology"],
    frequencyRank: "A",
    partOfSpeech: "noun",
    pronunciation: "/ɪkˈsperɪmənt/",
  },
  {
    id: 2129,
    word: "fundamental",
    meaning: "基本的な",
    example: "This is a fundamental concept.",
    exampleJa: "これは基本的な概念です。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 5,
    categories: ["school", "business"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/ˌfʌndəˈmentl/",
  },
  {
    id: 2130,
    word: "generate",
    meaning: "生み出す、発生させる",
    example: "The project will generate new jobs.",
    exampleJa: "そのプロジェクトは新しい雇用を生み出します。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 4,
    categories: ["business", "technology"],
    frequencyRank: "A",
    partOfSpeech: "verb",
    pronunciation: "/ˈdʒenəreɪt/",
  },
  {
    id: 2131,
    word: "genuine",
    meaning: "本物の、真の",
    example: "Is this a genuine product?",
    exampleJa: "これは本物の製品ですか？",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
    ],
    difficulty: 4,
    categories: ["shopping", "daily"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/ˈdʒenjuɪn/",
  },
  {
    id: 2132,
    word: "guarantee",
    meaning: "保証する",
    example: "We guarantee customer satisfaction.",
    exampleJa: "顧客満足を保証します。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic600", displayName: "TOEIC600" },
    ],
    difficulty: 4,
    categories: ["business", "shopping"],
    frequencyRank: "A",
    partOfSpeech: "verb",
    pronunciation: "/ˌɡærənˈtiː/",
  },
  {
    id: 2133,
    word: "implement",
    meaning: "実行する、実施する",
    example: "We will implement the new policy.",
    exampleJa: "新しい方針を実施します。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "toeic", level: "toeic700", displayName: "TOEIC700" },
    ],
    difficulty: 5,
    categories: ["business", "technology"],
    frequencyRank: "A",
    partOfSpeech: "verb",
    pronunciation: "/ˈɪmplɪment/",
  },
  {
    id: 2134,
    word: "imply",
    meaning: "暗示する、意味する",
    example: "What are you implying?",
    exampleJa: "何を暗示しているのですか？",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
    ],
    difficulty: 5,
    categories: ["communication", "opinion"],
    frequencyRank: "A",
    partOfSpeech: "verb",
    pronunciation: "/ɪmˈplaɪ/",
  },
  {
    id: 2135,
    word: "incredible",
    meaning: "信じられないほどの",
    example: "The view was incredible.",
    exampleJa: "景色は信じられないほど美しかったです。",
    courses: [
      { courseType: "senior", level: "senior2", displayName: "高校2年" },
      { courseType: "eiken", level: "eiken2", displayName: "英検2級" },
      { courseType: "conversation", level: "convIntermediate", displayName: "中級" },
    ],
    difficulty: 4,
    categories: ["emotion", "daily"],
    frequencyRank: "A",
    partOfSpeech: "adjective",
    pronunciation: "/ɪnˈkredəbəl/",
  },
];
