      ...
      <div className="main-content px-3 py-2 flex flex-col">
        <div className="max-w-md w-full mx-auto flex flex-col h-full">
          ...
          <div className="flex-shrink-0 flex items-center gap-2 mb-2">
            ...
            <h1 className="text-lg font-bold text-slate-800">クイズ設定</h1>
          </div>
        ...
        <div className="max-w-md w-full mx-auto flex flex-col h-full">
          ...
          <div className="flex-shrink-0 text-center bg-white rounded-2xl shadow-card p-3 mb-2">
            ...
            <div className="flex items-center justify-center gap-2 mb-1.5">
              ...
              <div>
                <h1 className="text-xl font-bold text-slate-800">{message.text}</h1>
                <p className="text-slate-500 text-xs">セッション完了!</p>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-primary-50 to-accent-50 rounded-lg p-2">
              <div className="flex items-center justify-center gap-3">
                <div>
                  ...
                  <p className="text-slate-600 text-xs">正答率 {percentage}%</p>
                </div>
                {maxCombo >= 3 && (
          ...
          <div className="flex-1 overflow-y-auto min-h-0 mb-2">
            ...
            {newAchievements.length > 0 && (
              <div className="mb-2 bg-white rounded-xl shadow-card p-2">
                <p className="text-xs text-gray-500 mb-1.5">新しい実績を獲得!</p>
                  ...
                  {newAchievements.map((achievement) => (
                    ...
                    >
                      ...
                      <div className="text-left">
                        <p className="font-bold text-gray-900 text-xs">{achievement.name}</p>
      ...
      <div className="max-w-md w-full mx-auto flex flex-col h-full">
        ...
        <div className="flex-shrink-0 mb-1">
          ...
          <div className="flex justify-center gap-1.5">
            <div className="inline-flex items-center gap-1 bg-white rounded-full px-2 py-0.5 shadow-card border border-primary-100">
              <span className="text-sm emoji-icon">✨</span>
              <span className="font-bold text-primary-500 text-xs">{score}</span>
              <span className="text-slate-400 text-[10px]">正解</span>
            </div>
            {combo >= 2 && (
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          <Card className="flex flex-col p-2 h-full">
            ...
            {currentQuestion.word.category && (
              <div className={`w-full h-7 mb-1 rounded-md bg-gradient-to-br ${getCategoryGradient(currentQuestion.word.category)} flex items-center justify-center border border-slate-100`}>
                <span className="text-lg emoji-icon">{CATEGORY_EMOJIS[currentQuestion.word.category] || "📝"}</span>
              </div>
            )}

            <div className="text-center mb-1">
              <p className="text-[10px] text-slate-400 mb-0.5">{getQuestionPrompt(currentQuestion.type)}</p>
                ...
                return (
                  <div className="mt-1 text-center">
                    ...
                    {showTranslation && translationInfo.sentenceJa && (
                      <p className="mt-0.5 text-[10px] text-slate-600 bg-slate-50 rounded-md p-1.5">
                        {translationInfo.sentenceJa}
                      </p>
                    )}
                    {showTranslation && !translationInfo.sentenceJa && (
                      <p className="mt-0.5 text-[10px] text-slate-400 bg-slate-50 rounded-md p-1.5">
                        （和訳データなし）
                      </p>
                    )}
                  </div>
            ...
            <div className={`${isCorrect ? "animate-pop-in" : "animate-shake"}`}>
              <Card className={`mb-1 p-1.5 ${
                ...
                {!isCorrect && currentQuestion.word.example && (() => {
                  const translationInfo = getTranslationInfo(currentQuestion.word.id, currentQuestion.word.example);
                  return (
                    <div className="mt-1 text-[10px] bg-white/70 rounded p-1 border border-slate-200">
