...
ローグライク方式で冒険に失敗すると所持アイテムをすべて失い、Lv1から再スタートする仕組みで繰り返し英語学習ができる。
...
| アイテム | Word/Spell/Bolt/食料/壷/特殊の6カテゴリ |
...
**Phase 8: プレイヤー向き表示・向き変え・射線化 ✅ 完了**
- `drawPlayer` に `dir` 引数を追加。4方向ピクセルアート（正面/後ろ姿/左右横顔）を実装
- `changeFacing(dx, dy)`: `playerDir` を更新して `redraw` + `saveGame`（敵ターン消費なし）
- 向き変え操作: PC は Shift+方向キー、スマホはコントローラー「↻」ボタン（タップで向きモードON → 次の方向入力で向き変更して自動OFF）
- 火炎草（drink）: `adjEnemy` から `lineEnemy()`（`playerDir` 方向直線上の最初の敵）に変更。隣接不要、外れても消費
- 杖: `adjEnemy` から `lineEnemy()` に変更。敵がいなくてもチャージ消費（ターン消費）
- 杖を投げる: 敵に当たると 70% で効果発動。外れたら床に落ちる（拾い直し可能）
