...
### Phase 3: 学習体験の強化 - 実装完了

| 機能 | 説明 | 状態 |
|------|------|------|
| SRS インページクイズ | /review ページ内で完結するクイズ形式の復習（list→quiz→resultの3フェーズ） | ✅ |
| SRS 間隔即時更新 | 回答ごとに SM-2 アルゴリズムで復習間隔をリアルタイム更新 | ✅ |
| 不正解再挑戦 | 結果画面から不正解単語だけを再度クイズできる | ✅ |
| 復習セッション復元 | 単語詳細に遷移して戻った際に同日中は結果画面を復元 | ✅ |
| 今日の単語 | 日付シードで毎日3語をランダムに選出し、ホーム画面に表示 | ✅ |
...
| タイプ | 説明 | デフォルト出題比率 |
|--------|------|----------|
| Type A: 英語→日本語 | 英単語を見て日本語の意味を選ぶ | 25% |
| Type B: 日本語→英語 | 日本語を見て英単語を選ぶ | 25% |
| Type C: リスニング | 例文音声を聞いて空欄に入る単語を4択で選ぶ | 25% |
| Type D: 書き取り | 例文音声を聞いて空欄に入る英単語をキーボードで入力 | 25% |

各タイプの出題比率はクイズ設定画面のスライダーで調整可能（0〜100の重みづけ、内部で正規化）。例文のない単語はリスニング/書き取りを除外して英→日/日→英にフォールバック。
...
| クイズに挑戦 | 日→英 | ❌ | なし | - |
| クイズに挑戦 | リスニング | ✅ | 例文全文 | 300ms |
| クイズに挑戦 | 書き取り | ✅ | 例文全文 | 300ms |
| スピードチャレンジ | 英→日 | ✅ | 英単語 | 100ms |
| スピードチャレンジ | 日→英 | ❌ | なし | - |

> **Note**: リスニング・書き取り問題では例文全体を自動再生する。ユーザーは音声ボタンで手動再生も可能。
...
| コアイメージ | 単語の核となるイメージ | 🔜 |
| 使い方 | 具体的な使用方法 | 🔜 |
| 関連語・対義語 | 品詞・意味付きリスト（タップで詳細遷移可能） | ✅ |
| 類義語との違い | 使い分けの解説（構造化リスト・テキスト内単語リンク付き） | ✅ |
| 英英定義 | 英語による定義 | 🔜 |
| 語源 | 単語の由来 | 🔜 |
| コラム | 学習に役立つ豆知識（テキスト内単語リンク付き） | ✅ |
...
全4,240語を一覧表示し、検索・フィルター・ソート機能で効率的に単語を探せる画面。フラッシュカードモードでSRS連動の自己評価学習も可能。
...
| 操作 | 動作 |
|------|------|
| カードタップ | 表面↔裏面の切り替え（3Dフリップアニメーション） |
| 左スワイプ | 次のカードへ |
| 右スワイプ | 前のカードへ |
| 「知ってた 👍」 | 正解として SRS 記録を更新し次のカードへ |
| 「わからなかった 😓」 | 不正解として SRS 記録を更新し次のカードへ |
| 「スキップ（評価なし）」 | SRS 記録を更新せず次のカードへ |
| 「詳細を見る →」 | `/word/[id]?from=wordlist` に遷移（戻ると状態復元） |
| 全カード評価完了 | セッション完了画面を表示 |

#### 開始方法

- **全件モード**: 右上「🃏 フラッシュカード」ボタン（フィルター適用済みの全単語）
- **特定単語から**: 単語行の「🃏」ボタン（その単語を先頭に同フィルターで開始）

#### セッション状態の復元

「詳細を見る」で離脱→戻ってきた際に以下の状態を復元:
- 現在のカードインデックス
- 全フィルター設定（コース・ステージ・カテゴリ・難易度・記憶度・ブックマーク・検索クエリ・ソート）

`flashcard-session.ts` の `saveFlashcardSession` / `getFlashcardSession` を利用（30分有効期限）。
...
- **ストレージ**: localStorage と Supabase のハイブリッド
- **ユーザー識別**: ログイン時は Supabase のユーザーID、未ログイン時は端末ローカル
- **同期**: ログイン時にローカルデータをSupabaseへ同期し、同期完了後はSupabaseを優先

### 7.2 認証

- **現在**: Supabase Auth を導入済み
- **ログイン/登録**: `/login`, `/register`
- **パスワード再設定**: `/reset-password`（メール送信 → リンク経由で新パスワード設定）
...
- **現在**: `src/data/words/*.ts` にコース別で静的定義
- **互換レイヤー**: 旧UI向けに `src/data/words/compat.ts` で変換
- **将来**: DB管理、外部API連携予定
...
アプリはログイン状態とデータ同期状態に応じて、データの保存先を自動的に切り替えます：
...
| WordColumn | コラム表示（テキスト内単語リンク付き） |
| WordPlaceholderSection | プレースホルダーセクション（テキスト内単語リンク付き） |
| WordText | テキスト内英単語リンク化（語彙DB照合・現在単語の青色表示） |
...
| パスワード再設定 | /reset-password | メール送信・再設定フォーム |
